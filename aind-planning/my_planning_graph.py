...
class PlanningGraph():
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pc1 in node_a1.parents:
            for pc2 in node_a2.parents:
                if pc1.is_mutex(pc2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for pc1 in node_s1.parents:
            for pc2 in node_s2.parents:
                if not pc1.is_mutex(pc2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            try:
                s_level = [x for x in self.s_levels if PgNode_s(goal, True) in x][0]
                level = self.s_levels.index(s_level)
                level_sum += level
            except:
                pass
        return level_sum
