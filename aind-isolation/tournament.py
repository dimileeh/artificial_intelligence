...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("Z-test")
    print("AB_Improves vs. AB_Custom")
    conf_interval_agent_0 = ztest.proportion_confint(total_wins[test_agents[0].player],
                                            total_matches,
                                            method = 'wilson')
    conf_interval_agent_1 = ztest.proportion_confint(total_wins[test_agents[1].player],
                                            total_matches,
                                            method = 'wilson')
    print('95%% confidence interval for a win probability, agent 0: [{}]'.format(conf_interval_agent_0))
    print('95%% confidence interval for a win probability, agent 1: [{}]'.format(conf_interval_agent_1))
    print("95%% confidence interval for the difference between proportions of wins: [{}]".format(\
      ztest.proportions_diff_confint_ind(total_wins[test_agents[0].player], \
                    total_wins[test_agents[1].player], total_matches)))

    print("p-value, one-sided, agent 1 is better than agent 0: {}".format(ztest.proportions_diff_z_test( \
                               ztest.proportions_diff_z_stat_ind(\
                                     total_wins[test_agents[0].player], \
                                     total_wins[test_agents[1].player], \
                                     total_matches), 'less')))
    print("p-value, two-sided, agent 1 is different from agent 0: {}".format(ztest.proportions_diff_z_test( \
                               ztest.proportions_diff_z_stat_ind(\
                                     total_wins[test_agents[0].player], \
                                     total_wins[test_agents[1].player], \
                                     total_matches))))

    print("\nAB_Improves vs. AB_Custom_2")
    conf_interval_agent_0 = ztest.proportion_confint(total_wins[test_agents[0].player],
                                            total_matches,
                                            method = 'wilson')
    conf_interval_agent_1 = ztest.proportion_confint(total_wins[test_agents[2].player],
                                            total_matches,
                                            method = 'wilson')
    print('95%% confidence interval for a win probability, agent 0: [{}]'.format(conf_interval_agent_0))
    print('95%% confidence interval for a win probability, agent 2: [{}]'.format(conf_interval_agent_1))
    print("95%% confidence interval for the difference between proportions of wins: [{}]".format(\
      ztest.proportions_diff_confint_ind(total_wins[test_agents[0].player], \
                    total_wins[test_agents[2].player], total_matches)))

    print("p-value, one-sided, agent 2 is better than agent 0: {}".format(ztest.proportions_diff_z_test( \
                               ztest.proportions_diff_z_stat_ind(\
                                     total_wins[test_agents[0].player], \
                                     total_wins[test_agents[2].player], \
                                     total_matches), 'less')))
    print("p-value, two-sided, agent 2 is different from agent 0: {}".format(ztest.proportions_diff_z_test( \
                               ztest.proportions_diff_z_stat_ind(\
                                     total_wins[test_agents[0].player], \
                                     total_wins[test_agents[2].player], \
                                     total_matches))))
